	...
	RandomUpgradeable
{
	...
	mapping(uint256 => Duckie) public tokenIdToDuckie;
	...
	function initialize(address duckiesAddress) public initializer {
		__ERC721_init('Yellow Duckies NFT Collection', 'YDNC');
		...
		return
			bytes(apiBaseURL).length > 0
				? string(
					abi.encodePacked(
						apiBaseURL,
						tokenIdToDuckie[tokenId].gene.toString(),
						'-',
						uint256(tokenIdToDuckie[tokenId].birthdate).toString()
					)
				)
				: '';
	}
	...
	function duckiesPerMint() external view returns (uint256) {
		return _duckiesPerMint();
	}

	function mintPack(uint8 amount) external UseRandom {
		require(amount <= MAX_MINT_PACK_SIZE, 'pack size exceeded');

		duckiesContract.burnFrom(msg.sender, _duckiesPerMint() * amount);
	...
	function meld(uint256[5] calldata meldingTokenIds) external UseRandom {
		...
		tokenIdToDuckie[meldedTokenId] = Duckie(meldedGene, uint64(block.timestamp));
	...
	function _mint() internal returns (uint256 tokenId, uint256 gene) {
		...
		tokenIdToDuckie[tokenId] = Duckie(gene, uint64(block.timestamp));
	...
	function _duckiesPerMint() internal view returns (uint256) {
	...
	function _burnTokensAndGetGenes(
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			genes[i] = tokenIdToDuckie[tokenIds[i]].gene;
			_burn(tokenIds[i]);
		}
