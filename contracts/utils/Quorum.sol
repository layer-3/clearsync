...
contract Quorum {
    ...
    Counters.Counter public quorumConfigurationNonce;
    // TODO: make public?
    ...
    function addValidator(address newValidator, bytes[] calldata signatures) external {
        ...
        requireQuorum(
            abi.encode(
                ...
                quorumConfigurationNonce.current(),
                newValidator
            ),
            signatures
        );
        ...
        quorumConfigurationNonce.increment();
    ...
    function setQuorum(uint8 newQuorum, bytes[] calldata signatures) external {
        ...
        requireQuorum(
            abi.encode(
                QuorumConfigurationChange.setQuorum,
                quorumConfigurationNonce.current(),
                newQuorum
            ),
            signatures
    ...
    function setQuorumConfiguration(
        ...
        requireQuorum(
            abi.encode(newValidators, newQuorum, quorumConfigurationNonce.current()),
            signatures
        );
