...
func newKraken(config KrakenConfig, outbox chan<- common.TradeEvent, history HistoricalData) (Driver, error) {
...
func (k *kraken) subscribeUnchecked(market common.Market) error {
...
func (*kraken) HistoricalData(_ context.Context, _ common.Market, _ time.Duration, _ uint64) ([]common.TradeEvent, error) {
	return nil, errors.New("not implemented")
}

func (k *kraken) unsubscribeUnchecked(market common.Market) error {
	pair := fmt.Sprintf("%s/%s", strings.ToUpper(market.Base()), strings.ToUpper(market.Quote()))
	if _, ok := k.availablePairs.Load(pair); !ok {
		return fmt.Errorf("market %s doesn't exist in Kraken", pair)
	}
...
func (k *kraken) listen() {
	for {
		...
		if err != nil {
			...
			k.streams.Range(func(market common.Market, _ struct{}) bool {
				if err := k.subscribeUnchecked(market); err != nil {
...
func (*kraken) buildEvents(trades krakenTrade) ([]common.TradeEvent, error) {
	events := make([]common.TradeEvent, 0, len(trades.TradeData))
	for _, tr := range trades.TradeData {
		...
		market, ok := common.NewMarketFromString(trades.Pair)
		if !ok {
