...
type SyncswapConfig struct {
	...
	StablePoolMarkets         []string     `yaml:"stable_pool_markets" env:"STABLE_POOL_MARKETS" env-default:"usdc/usdt"` // `env-default` tag value is a comma separated list of markets as in `usdc/usdt,usdc/dai`
	Filter                    FilterConfig `yaml:"filter" env-prefix:"FILTER_"`
}
