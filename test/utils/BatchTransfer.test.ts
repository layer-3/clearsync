...
describe('BatchTransfer', function () {
  ...
  before(async () => {
    [TokenAdmin, BatchTransferOwner, ...Receivers] = await ethers.getSigners();
  });
  ...
  it('Successful ERC20 transfers', async () => {
    ...
    expect(await Token.balanceOf(Batcher.address)).to.equal(
      BATCHER_BALANCE - amount * receivers.length,
    );

    for (const [i, receiver] of receivers.entries()) {
      expect(await Token.balanceOf(receiver)).to.equal(tokenBalances[i].add(amount));
    }
  });
  ...
  it('Successful native token transfers', async () => {
    ...
    expect(await Batcher.provider.getBalance(Batcher.address)).to.equal(
      BATCHER_BALANCE - amount * receivers.length,
    );
  ...
  it('Insufficient ERC20 balance', async () => {
    const amount = 2000;
    const receivers = Receivers.slice(0, 20).map((r) => r.address);
    await expect(Batcher.batchTransfer(Token.address, receivers, amount)).to.be.revertedWith(
    ...
    for (const [i, receiver] of receivers.entries()) {
      expect(await Token.balanceOf(receiver)).to.equal(tokenBalances[i]);
    }
  });
  ...
  it('Insufficient native token balance', async () => {
    ...
    const receivers = Receivers.slice(0, 20).map((r) => r.address);
    await expect(
      Batcher.batchTransfer(ethers.constants.AddressZero, receivers, amount),
    ).to.be.revertedWith('Contract has insufficient balance.');
  ...
  it('Unknown token', async () => {
    ...
    await expect(Batcher.withdraw(UnknownToken.address)).to.be.revertedWith(
      'Contract has no balance of such token.',
    );
  });
});
